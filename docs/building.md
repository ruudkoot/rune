...
| `make check-docs` | verify docs, tests and `.def` files are in sync, that the Basis Library suite has a check for every specified member, that the library's signatures have the tokens of their transcriptions, that the comments of `lib/basis` and `src` are in the language of doc comments (`runedoc --lint`, [doc-comments.md](doc-comments.md)), that `docs/generated/basis` is up to date, and that the structures the library says implement a signature are the ones the suite matches against it (`tests/basis/check-claims.sh`) |
