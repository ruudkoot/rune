...
| basis.os.io | `OS.IO` (`RuneIODesc`): `iodesc`, `hash`, `compare`, `iodesc_kind`, `kind`, `Kind`, `poll_desc`, `poll_info`, `pollDesc`, `pollToIODesc`, `Poll`, `pollIn`, `pollOut`, `pollPri`, `poll`, `isIn`, `isOut`, `isPri`, `infoToPollDesc` | Supported | A descriptor is a handle of the VM's file table; `poll` is the POSIX one. |
...
| basis.widetext | `WideText`: `Char`, `String`, `Substring`, `CharVector`, `CharArray`, `CharVectorSlice`, `CharArraySlice` of the wide character | Supported | As `Text` is of `char`. There is no `WideTextIO` (and no host has one). |
