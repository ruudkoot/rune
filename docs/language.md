...
| basis.socket | `Socket` (`RuneSocket`): `sock`, `sock_addr`, `AF`, `SOCK`, `socket`, `socket'`, `socketPair`, `socketPair'`, `bind`, `connect`, `listen`, `accept`, `close`, `shutdown`, `getSockName`, `getPeerName`, the `sendVec`/`sendArr`/`recvVec`/`recvArr` families with their `To`/`From` and flag forms, the options (`REUSEADDR`, `KEEPALIVE`, `SNDBUF`, ...), `select`, `sockDesc`, `ioDesc` | Partial | A socket is a handle of the system and an address the bytes it keeps one in. `getNREAD` and `getATMARK` answer 0 and `false`; `getLINGER` reports only whether lingering is on. `select` is the `poll` of `OS.IO`. |
