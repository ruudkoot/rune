...
| basis.io | `IO`: exceptions `Io {name, function, cause}`, `BlockingNotSupported`, `NonblockingNotSupported`, `RandomAccessNotSupported`, `ClosedStream`; datatype `buffer_mode`; signatures `PRIM_IO`, `STREAM_IO`; `TextPrimIO`, `BinPrimIO`; `Position`; the functors `RunePrimIOFn`, `RuneStreamIOFn`, `RuneImperativeIOFn` and `RuneFile`, which holds the VM's file handles | Supported | A stream on a file hands its output to the VM, which buffers the file, and flushes it as its buffer mode says (`openOut` is `BLOCK_BUF`, `LINE_BUF` on a terminal; `stdErr` is `NO_BUF`). A stream a program makes over a writer of its own buffers as its mode says, and what it holds is flushed when the program ends or calls `OS.Process.exit` (not `Posix.Process.exit`). `augmentReader` and `augmentWriter` fill in every row of the specification's table. |
