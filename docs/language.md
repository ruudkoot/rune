...
| basis.runtime | `Runtime` (`RuneRuntime`), signature `RUNTIME`: `stats` | Supported | Not in the specification, which says nothing about the implementation a program runs on. `stats` is the counters the VM already keeps and `runevm --count` and `--stats` print when a program ends: instructions executed, bytes and objects allocated, collections made, the bytes of the current semispace in use and the size of one semispace. Each is a primitive that reads one field and allocates nothing, so the five of the heap cannot drift apart while they are read. The first four depend on the program and its input alone. The structure is `host = no` in the MANIFEST: no other SML system counts any of this. |
