...
| ty.overload.literal | Overloaded integer and word constants (in expressions and patterns) and operators at the types registered by the basis library: `IntInf.int`; real constants and operators at `Real32.real` | Supported | Default `int`/`word`; out-of-range constants are compile-time errors (`tests/errors/err.literal_*`). |
...
| basis.mono.widechar | `WideCharVector`, `WideCharVectorSlice`, `WideCharArray`, `WideCharArraySlice`: the same, with `elem = WideChar.char` | Supported | Applications of the functors of `basis.mono.bool` (`lib/basis/widechar.sml`); the specification has no `WideCharArray2`. |
