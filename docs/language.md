...
| basis.os.process | `OS.Process`: `status`, `success`, `failure`, `isSuccess`, `system`, `atExit`, `exit`, `terminate`, `getEnv`, `sleep`; `OS.SysErr`, `OS.syserror`, `OS.errorMsg`, `OS.errorName`, `OS.syserror`; `OS.IO.iodesc` | Supported | A `syserror` is an `errno` value. `system` reports what the command exited with, or 128 plus the signal that ended it. The actions of `atExit` run when the program ends or calls `exit`, not when it calls `terminate` and not after an uncaught exception; `RuneError` holds the errors of the system, `RuneExit` the actions and `lib/basis/epilogue.sml`, compiled after a program that uses `OS`, runs them. |
...
| basis.netdb | `NetHostDB` (`RuneNetHostDB`): `in_addr`, `entry`, `name`, `aliases`, `addrType`, `addr`, `addrs`, `getByName`, `getByAddr`, `getHostName`, `toString`, `scan`, `fromString`; `NetProtDB` (`RuneNetProtDB`) and `NetServDB` (`RuneNetServDB`) with their entries | Supported | An `in_addr` is the dotted text of an address. |
