...
| basis.mono.real64 | `Real64Vector`, `Real64VectorSlice`, `Real64Array`, `Real64ArraySlice`, `Real64Array2`: the same, with `elem = Real64.real` | Supported | The structures of `basis.mono.real`: `Real64` is `Real` (`lib/basis/mono_real64.sml`). There is no `Real32`, and so no `Real32Vector`. |
...
| basis.pack | `PackWord16Big`, `PackWord16Little`, `PackWord32Big`, `PackWord32Little`, `PackWord64Big`, `PackWord64Little` (`RunePackWordFn`): `bytesPerElem`, `isBigEndian`, `subVec`, `subVecX`, `subArr`, `subArrX`, `update`; `PackRealBig`, `PackRealLittle`, `PackReal64Big`, `PackReal64Little` (`RunePackRealFn`): `toBytes`, `fromBytes`, `subVec`, `subArr`, `update`; `Real64` | Supported | A real is packed as the 8 bytes of IEEE 754 binary64 (the primitives `real_to_bits` and `real_from_bits`); `Real64` is `Real`, and there is no `Real32`, so no `PackReal32*`. |
