...
| basis.socket | `Socket` (`RuneSocket`): `sock`, `sock_addr`, `dgram`, `stream`, `passive`, `active`, `AF`, `SOCK`, `Ctl` (the options, `getPeerName`, `getSockName`, `getNREAD`, `getATMARK`), `sameAddr`, `familyOfAddr`, `bind`, `listen`, `accept`, `acceptNB`, `connect`, `connectNB`, `close`, `shutdown`, `sock_desc`, `sockDesc`, `sameDesc`, `select`, `ioDesc`, `out_flags`, `in_flags`, and `sendVec`, `sendArr`, `recvVec`, `recvArr` with their `'`, `NB`, `To` and `From` forms | Partial | A socket is a descriptor of the system and an address the bytes of its `sockaddr`. The `NB` forms switch the descriptor to non-blocking mode for the call. `Ctl.getNREAD` and `Ctl.getATMARK` answer 0 and `false`; `Ctl.getLINGER` reports only whether lingering is on, and `Ctl.setLINGER` fails (the primitive passes an `int`, the system wants a `struct linger`). `select` is the `poll` of `OS.IO`. A receive of zero bytes returns at once. A send to a peer that has gone ends the program with `SIGPIPE`, which Rune does not ignore. |
...
| basis.io | `IO`: exceptions `Io {name, function, cause}`, `BlockingNotSupported`, `NonblockingNotSupported`, `RandomAccessNotSupported`, `ClosedStream`; datatype `buffer_mode`; signatures `PRIM_IO`, `STREAM_IO`; `TextPrimIO`, `BinPrimIO`; `Position`; the functors `RunePrimIOFn`, `RuneStreamIOFn`, `RuneImperativeIOFn` and `RuneFile`, which holds the VM's file handles | Supported | A stream on a file hands its output to the VM, which buffers the file, and flushes it as its buffer mode says (`openOut` is `BLOCK_BUF`, `LINE_BUF` on a terminal; `stdErr` is `NO_BUF`). A stream a program makes over a writer of its own buffers as its mode says; what such a `BLOCK_BUF` stream holds is lost if the program ends without flushing it. `augmentReader` and `augmentWriter` fill in every row of the specification's table. |
