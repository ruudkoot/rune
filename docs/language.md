...
| basis.socket | `Socket` (`RuneSocket`): `sock`, `sock_addr`, `dgram`, `stream`, `passive`, `active`, `AF`, `SOCK`, `Ctl` (the options, `getPeerName`, `getSockName`, `getNREAD`, `getATMARK`), `sameAddr`, `familyOfAddr`, `bind`, `listen`, `accept`, `acceptNB`, `connect`, `connectNB`, `close`, `shutdown`, `sock_desc`, `sockDesc`, `sameDesc`, `select`, `ioDesc`, `out_flags`, `in_flags`, and `sendVec`, `sendArr`, `recvVec`, `recvArr` with their `'`, `NB`, `To` and `From` forms | Supported | A socket is a descriptor of the system and an address the bytes of its `sockaddr`. The `NB` forms switch the descriptor to non-blocking mode for the call. `select` is the `poll` of `OS.IO`. A receive of zero bytes returns at once. A send to a peer that has gone fails with `EPIPE` (`MSG_NOSIGNAL`) instead of ending the program. |
