...
| Group | Primitives |
|---|---|
| Polymorphic | `poly_eq` (structural equality; refs/arrays/closures by identity), `ptr_eq` (identity) |
...
| Word (64-bit, wrapping) | `word_add word_sub word_mul word_div word_mod word_lt word_le word_gt word_ge word_andb word_orb word_xorb word_notb word_lsl word_lsr word_to_int word_to_int_x word_from_int word_to_string` — `word_div/word_mod` raise `Div`; `word_to_int` raises `Overflow`; `word_to_string` is uppercase hex |
...
| Ref / array / vector | `ref_new ref_get ref_set array_new array_length array_sub array_update array_from_list vector_from_list vector_length vector_sub` — `array_new` raises `Size`; indexing raises `Subscript` |
