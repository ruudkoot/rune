...
| Milestone | State |
|---|---|
| M0, this roadmap | done (`3f3aa91`): the signature below, four decisions, and every claim of *Where it stands* measured on that commit. **M1 to M9 are done**; the signature is as it was drafted, but for `frame` gaining nothing and `save` returning `world` as D3 proposed |
