...
| M8, elaboration | done: `DocElab` elaborates the library as the driver does and checks all 197 claims by ascription (1.2 s natively, about 5 s on `runevm`; a false claim fails with the compiler's message), usage heads are checked against elaborated types, `top-level.md` is generated from the files every program loads and the `val null = null` twins of the structures (each name with the description of the member it is), signature entries say when they are also at the top level, `exceptions.md` lists who raises what, unqualified top-level names such as `Subscript` resolve to the member of `General` they are. Value twins are found by syntax, as planned, and aliases are shown from the syntax too (`structures.md`). Done after M12: what a structure declares beyond its signature is taken from elaboration where the source shows no body (an alias, an application of a functor), with what the signature specifies taken from its elaborated environment, so that included members, replicated constructors and the inline signature of a substructure count; and [types.md](../generated/basis/types.md) is the type-equivalence table, the documented types grouped by the stamp of their type name (`int` is also `Time.time`, `OS.syserror` and `Posix.Process.pid`; `string` is also `Word8Vector.vector`, which got the note `Word8Vector.vector/is-string`) |
...
| M11, the waves W1 to W8 | done: all 65 signatures are documented in full and on the ratchet list, 1,538 of 1,538 entries and 1,078 of 1,078 functions with a checked usage head. W8 also wrote `lib/basis/overview.doc` and the description of the top level's `print`, and replaced the three sections "Representation choices in Rune", "Where Rune departs from the specification" and "Readings of the specification" of `docs/basis-compat.md` by links to [readings.md](../generated/basis/readings.md). Writing the pages found four library defects and a check that was too weak: `IEEEReal.Unordered` was an exception of its own where the specification asks for the top level's; `check-notes.sh` resolved a structure to one signature where it may claim several; six header notes were written `Transcription-fix:`, which the comment language does not reserve, and said nothing; the inventory's claim that a `TextIO` outstream does not admit equality is wrong. (That `WideString.scan` stops at an unescaped quote was taken for stale here and was true: see the audit below, after which it was changed.) The stale comments of `runefile.sml`, `unix.sml`, `posix_filesys.sml`, `docs/plans/basis.md` and `docs/language.md` (`WideTextIO`, `iodesc`) are corrected |
| M12, optional | done. **Host annotations**: `runedoc --annotations FILE` reads remarks `label-glob \| whom it is about \| text` and shows each, folded, under every member that has a check with such a label (a constructor counts for its datatype, `Structure:SIG/case` for the signature; an annotation that finds no check of a documented member is an error); `tests/basis/gen-annotations.sh` makes `tests/basis/annotations.txt` from the host lines of `deviations.txt` (343 annotations, 311 blocks "Other implementations"), it is committed and `make check-docs` keeps it up to date. A check whose label is computed counts by the beginning that is written out; 25 of the 343 globs match computed labels only. Two of those (`*.scan/DEC-whitespace-*`, `*.fromString/whitespace-*`) showed under `Bool.scan` and `Bool.fromString` as well, which is more than the hosts fail, so a label of which only the member is known counts only with reason: the words of the glob begin a part of a string constant of the test's file, and the text does not speak of another structure by name. **Examples that run**: a piece of an `Example:` that is an equation `e = v` is elaborated when the documentation is made, under `open S` for the required structure with the shortest name (`Int` for `INTEGER`), and `runedoc --examples DIR` writes a program for each signature that `make test-basis` compiles and runs (`tests/basis/run-examples.sh`; 26 examples of 10 signatures at first); a wrong example fails `make test-basis`, an ill-typed one `make docs`. **Installation**: `make install` installs `runedoc` (the same kind of build as `rune`) with `man/runedoc.1`, bash and zsh completions and the `overview.doc` and `DOCUMENTED` of the library; `--library` takes a directory, and a library other than `LIBDIR/basis` is elaborated on top of the Basis Library, so that an installed `runedoc` documents a library of the user's. Found on the way: code between backquotes lost its repeated blanks (`"Wed Mar  8 ..."` of `Date.toString` was shown with one), two examples of LIST had a free variable, and the README still said that `Real32` and `WideChar` were missing |
| After M12: the inventory, audited | done. The 440 rows of [docgen-notes.tsv](docgen-notes.tsv) that are notes or borderline were read against the comments of `lib/basis` by four readers, each claim they found missing or doubtful checked against the code or a run. 383 were there; the rest were not, or only in part, and are notes or prose now (328 notes in `readings.md`); 6 rows are stale or wrong and are dropped (402 and 455: the signatures are no longer generated and `check-basis-coverage.sh` is gone; 53, 120, 294 and 270 in part); the rows that only say how the suite picks its numbers (411, 413, 414, 438) stay in the tests. **Eleven descriptions written in M11 were false** and are corrected: `Date.toString` (the day is padded with a zero: `%d`, not `%e`), `Real.toDecimal` (the shortest digits that read back, not the exact expansion), `SML90.Quot` (division by zero raises `Mod`) and `SML90.Interrupt` (nothing raises it), `StreamIO.closeOut` (what the stream held is gone when the flush fails), `IEEEReal.scan` (a huge exponent is cut off above 10^8, not taken as the largest `int`), `Word.scan` (`0w` is no prefix in `HEX`), `WideChar.fromString` (`\ddd` is exactly three digits; C's octal and `\x` escapes stop at 255), `OS.IO.kind` (what is none of the others is a `device`), `OS.FileSys.modTime` (the VM drops the fraction, not the file system), and the misnamed id `PRIM_IO/pos-of-the-instances`. **Two library defects** are fixed, each with a test in `tests/lang`: `OS.Process.atExit` registered the actions that an action asks for and ran them, where the specification has such calls ignored (the suite's check writes a file that nothing reads); `Date.fmt` printed a year beyond C's 32-bit `int` as another year and raises `Date` now (the bound is worked out and not written as a constant: `make matrix-quick` showed that the 32-bit SML/NJ, whose `int` has 31 bits, refused `date.sml` with the constant in it and fell back to its own `Date`). The same run found a line of `deviations.txt` that the Word8 change had left stale (`xc1:smlnj | @section/word8/constants`: as one of the WordN family its constants load there, and `Word[0-9]*.+/wraps` explains the one check that differs); it is removed, and the matrix is green again: 0 failed in `rune` and the four `xc1` configurations. `WideSubstring` had no claim because `check-claims.sh` read an ascription on one line only. `WideString.scan` stopped at a double quote that no backslash precedes, where `String.scan` and `WideString.fromString` convert it, and a check of the suite pinned that with the quote as its "character it cannot read". The owner had it made consistent: the page ends a scan at the end of the source, at a character that does not print and at an improper escape, and a double quote is none of these, which is the reading `String.scan` already had. It converts now, the check uses the bell, and `WideString.scan/unescaped-double-quote*` pin the reading |
| After M12: the owner's decisions of 2026-09-21 | done. (1) `Char.fromString "\""` gives `NONE`, the last check that failed on Rune: the page of CHAR has the text read "as allowed in an SML program", that of STRING lists what stops a scan, and each is followed; `make matrix-quick` has 0 explained for `rune`. (2) Types are to be one type only where the specification has it so, and the structures sealed as far as the specification allows. [types.md](../generated/basis/types.md) says of every name why it is a name of its type: required by the signature (with the `where type` of the claim; `DocElab.typeSpecOf` asks the elaborator whether the signature leaves the type abstract), required of the top level, the implementation's choice (the structure is another by name), or abstract in the specification, which is a leak. The baseline before any sealing: 226 required by a signature, 5 of the top level, 33 by choice, **47 leaks**, and 40 structures with names beyond their signature. **Names beyond the signature: none left.** The MANIFEST has a fourth kind of file, `seal`: 69 files bind 109 structures again, each ascribed to its signature (`structure List : LIST = List`), and are loaded for a program that mentions the structure and for no file of the library, so the library keeps its helpers and nothing in it had to change; the other 60 structures were ascribed where they are declared. `runedoc` takes what a structure shows from elaboration and fails for a structure of a fully documented library that shows more than its signatures name. The whole suite passes on the sealed library; three tests of `tests/lang` used `Vector.toList`, `Array.toList` and `Posix.FileSys.defaultMode`, which are gone for a program together with `Time.micros`, `Unix.protect`, `StreamIO.mkOutstreamOver` and the socket errors of `Posix.Error`. The cost: compiling `hello` +9.6% (the MANIFEST has 69 lines more), the bootstrap +3.8%; nothing at run time. **Leaks: 47 to 11**, without a change to any structure. Nine were equalities that the specification requires and the claims did not say (the slices of the `PrimIO` structures, `Unix.exit_status` and `Unix.signal`, `Posix.FileSys.dirstream` and `access_mode`, which its page calls identical to those of `OS.FileSys`). The rest went by sealing opaquely in the seal file: a type that no other signature names becomes abstract with `:>`, and the types that other signatures do name are kept by `where type`, since those signatures were elaborated with the structure whole. So `Posix` is opaque but for `Signal.signal` and `Process.exit_status` (`Unix` has them), and `uid`, `gid`, `pid`, `file_desc`, `dev`, `ino`, `speed`, the actions of `TC`, `V.cc` and nine flag types are abstract; likewise `SML90`, `StringCvt.cs`, the timers, `Socket.sock_desc`, and in `OS` the `file_id`, `iodesc_kind`, `poll_desc` and `poll_info`. `types.md` has a second list, the abstract types that show what they are made of (a record, an application of a type): 64 at first, **56 now**, after `Date.date`, `OS.FileSys.file_id`, the entries of the network databases and `StreamIO.out_pos`. Two tests of `tests/lang` added a process id to an `int` and printed a mode as a `word`, and use `pidToWord` and `S.toWord` now. A type that several structures share has to be made abstract where it is declared, and four are: `Time` is sealed with `TIME` in `time.sml` (its signature comes before it in the MANIFEST now), with `micros` and `ofMicros` for the library, which the seal file hides; `RuneError.syserror`, `RunePosixSignal.signal` and the new `RuneStatus.status` are opaque with `toInt` and `fromInt`. Only `Timer` had used a time as an `int`, and eleven places an error number, a signal or a status; five tests of `tests/lang` had (`OS.Process.exit 3`, `Int.toString Posix.Signal.kill`) and use the specification's conversions now. **Leaks: 7.** What is still left: `NetHostDB.addr_family` and `in_addr` with `Socket.SOCK.sock_type` (`socket.sml` comes before `netdb.sml` and has an `int` of its own), `TextPrimIO.pos` and `WideTextPrimIO.pos`, `Word8Vector.vector` with its slice (strings, which `Byte` and `BinIO` rely on), and the 54 monomorphic vectors and arrays, which are polymorphic ones of their elements. **Leaks: 2.** `NetHostDB.addr_family` and `in_addr` with `Socket.SOCK.sock_type` are declared in a new `runenet.sml`, which comes before `socket.sml`, and `TextPrimIO.pos` and `WideTextPrimIO.pos` in a new `runepos.sml`, as two types of their own; `RuneStreamIOFn` takes `advance` instead of demanding `pos = int`, which was the only arithmetic it did on a position. Three tests of `tests/lang` made an address from a string and one of `tests/basis` made a position from an `int`; they go through `NetHostDB.fromString` and through a reader now. Five alias structures had no claim of their own and so no name in `types.md` at all (`Real64`, `LargeWord`, `SysWord`, `FixedInt`, `Position`): each has one now, and the suite matches it against the specification's signature, which is why the names by the implementation's choice read 37 and not 33. What is left for the owner to decide is in [basis-types-review.md](basis-types-review.md): the 37 by choice one by one, and the measured cost of the last two leaks (`Word8Vector.vector` with its slice) and of the 54 monomorphic vectors and arrays. The monomorphic ones are free (identical instruction counts); the byte vector costs 0.80% on a worst case when it is sealed file by file and 0.00014% when the vector and its slice are sealed as one group. **All of it is carried out**: the leaks are 0, the types that show a record or an application are 0, and the names that are one type by the implementation's choice are 20 (`Position`, the reals, `LargeWord` and `SysWord` with their families). `Array2` could not be sealed and then could: its page asks for three things that Standard ML cannot give together, written down as the erratum `ARRAY2/sealed-and-equal-at-any-element` and in docs/basis-compat.md under a heading of its own. The public `StreamIO` functor lost its deviation too (`filePosIn` reads the chunk again where it cannot count in a position, as MLton's does), `Int64` and `Word64` became types of their own so that a VM may choose the width of `Int` and `Word`, and the claims of `WideText` and `LargeInt` now say what the specification requires |
...
* The acceptance tests named in the milestone table: a changed token, a
  ...
  the place.
* Run on 2026-09-20 after M12, in a scratch copy, each change by itself, and
  each fails `make check-docs` (or, for the example, `make test-basis`) with
  a message that names the place: a changed token of `sig_list.sml`
  (`gen-basis-sigs.sh --check`), a removed `Implements: LIST` (the references
  to `List.foldl` lead nowhere), `Implements: LIST` on `Bool` (the compiler's
  signature mismatch), a reference to `List.nothingAtAll`, the note of
  `Char.fromString` turned into prose and the `rune` line of `deviations.txt`
  deleted (`check-notes.sh`, either way), a `Pinned by:` without a check, a
  comment changed without `make docs` (`runedoc --check`), a host line of
  `deviations.txt` changed without `gen-annotations.sh`, an annotation whose
  check does not exist, an example with a wrong value and one with a wrong
  type.
