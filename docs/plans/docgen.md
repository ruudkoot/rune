...
| After M12: the owner's decisions of 2026-09-21 | in progress. (1) `Char.fromString "\""` gives `NONE`, the last check that failed on Rune: the page of CHAR has the text read "as allowed in an SML program", that of STRING lists what stops a scan, and each is followed; `make matrix-quick` has 0 explained for `rune`. (2) Types are to be one type only where the specification has it so, and the structures sealed as far as the specification allows. [types.md](../generated/basis/types.md) says of every name why it is a name of its type: required by the signature (with the `where type` of the claim; `DocElab.typeSpecOf` asks the elaborator whether the signature leaves the type abstract), required of the top level, the implementation's choice (the structure is another by name), or abstract in the specification, which is a leak. The baseline before any sealing: 226 required by a signature, 5 of the top level, 33 by choice, **47 leaks**, and 40 structures with names beyond their signature |
