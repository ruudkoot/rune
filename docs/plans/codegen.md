...
| M8, the round-up | not started |
| M9, save and restore (optional) | not started |
...
its own working directory as `DW_AT_comp_dir`. So the tool takes an option
for the source directory. For the same reason the cross-host check compares
the `.s` files, not the executables.
