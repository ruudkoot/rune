...
val () = print "\n"
val () = print (String.concatWith "," (map Int.toString (Vector.toList (Vector.map (fn x => x + 1) v))) ^ String.concatWith "," (map Int.toString (Vector.toList (Vector.mapi (fn (i, x) => i + x) v))) ^ "\n")
