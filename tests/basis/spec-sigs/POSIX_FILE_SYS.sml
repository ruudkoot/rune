(* signature POSIX_FILE_SYS, transcribed from
   https://smlfamily.github.io/Basis/posix-file-sys.html

   Uses SPEC_BIT_FLAGS (spec-sigs/BIT_FLAGS.sml), which has to be loaded
   first: the substructures S and O include BIT_FLAGS.
   ...
   Posix.ProcEnv.uid and gid, and file_desc is Posix.ProcEnv.file_desc (the
   constraints of `structure FileSys : POSIX_FILE_SYS` in signature POSIX),
   dirstream is "identical to OS.FileSys.dirstream" and access_mode to
   OS.FileSys.access_mode; posix_filesys_sig.sml checks those identities. *)
signature SPEC_POSIX_FILE_SYS =
sig
  eqtype uid
  eqtype gid
  eqtype file_desc
