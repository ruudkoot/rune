(* signature POSIX_PROCESS, transcribed from
   https://smlfamily.github.io/Basis/posix-process.html

   Uses SPEC_BIT_FLAGS (spec-sigs/BIT_FLAGS.sml), which has to be loaded
   first: the page specifies `structure W : sig include BIT_FLAGS val untraced
   : flags end`. The type signal is left flexible, as on the page; POSIX fixes
   it to Signal.signal (spec-sigs/POSIX.sml). *)
signature SPEC_POSIX_PROCESS =
sig
  eqtype signal
  eqtype pid
