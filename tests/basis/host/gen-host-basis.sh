...
# matching, and no host can give either a type. `Runtime.same` is the one
# member of the library that names ptr_eq, and lib/basis/runtime.sml is
# `host = no`, so no source a host compiles names either of them.
awk '
BEGIN {
