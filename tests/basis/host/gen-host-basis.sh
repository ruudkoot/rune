...
# poly_eq, imm_eq and ptr_eq are what the compiler makes of `=` and of
# exception matching, and no host can give any of them a type. `Runtime.same`
# is the one member of the library that names ptr_eq, and
# lib/basis/runtime.sml is `host = no`, so no source a host compiles names
# any of them.
awk '
BEGIN {
...
/^[[:space:]]*#/ || /^[[:space:]]*$/ { next }
$1 == "poly_eq" || $1 == "imm_eq" || $1 == "ptr_eq" { next }
{
  line = $0
  sub(/^[^ \t]+[ \t]+[^ \t]+[ \t]+/, "", line)
  n = match(line, /  |\t/)
