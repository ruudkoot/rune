...
struct
  ...
  fun readme (env : P.env, title : string, overview : I.doc, sigs : I.signatureRecord list, letters : string list) : string =
    let
    ...
    in
      ...
      ^ "[How to read these pages](conventions.md) &middot; [what is documented](coverage.md) &middot; index: "
  ...
  fun build {dir : string, title : string, out : string} : file list =
    let
      ...
      val index = R.indexOf modules
      val links = ref []
      val anchors = ref []
      fun env root : P.env = {index = index, root = root, up = upFrom out, links = links, anchors = anchors}
      ...
      val files =
        ("README.md", readme (env "", title, overviewOf dir, sigs, letters))
